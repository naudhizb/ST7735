...
#define TFT_CS                  (*((volatile uint32_t *)0x40058020))
#define TFT_CS_LOW              0           // CS normally controlled by hardware
#define TFT_CS_HIGH             0x08
#define DC                      (*((volatile uint32_t *)0x40058100))
#define DC_COMMAND              0
#define DC_DATA                 0x40
#define RESET                   (*((volatile uint32_t *)0x40058200))
#define RESET_LOW               0
...
void static writecommand(unsigned char c) {
	ST7735p_Send(DC_COMMAND, c);
}


void static writedata(unsigned char c) {
	ST7735p_Send(DC_DATA, c);
}
...
void ST7735_OutChar(char ch){
  ...
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',StTextColor,ST7735_BLACK, 1);
  }
  return;
}
//********ST7735_OutString*****************
